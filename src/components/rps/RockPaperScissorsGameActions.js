...
export function portisPlay(value, computerChoice) {
  let web3 = store.getState().web3.web3Portis;
  let portis = store.getState().web3.portis;
  ...
  if (typeof web3 !== 'undefined') {
    ...
    return function(dispatch) {
      // Using truffle-contract we create the RockPaperScissors object.
      const rps = contract(RockPaperScissorsContract);
      rps.setProvider(portis.provider);
